...
public:
    ...
    void setDisciplinaProf(weak_ptr<Persoana> p){
        prof = p;
    }
    void setNume(string n){
        nume = n;
    }
    string printProfesor(){
 ...
 */
    Facultate& addPersoane(shared_ptr<Persoana> p){persoane.push_back(p); return *this;}
    Facultate& addDisciplina(shared_ptr<Disciplina> d){discipline.push_back(d); return *this;}
    ...
    shared_ptr<Persoana> getProfesor(int idProf){
        int ctr=0;
        for (int i=0; i<persoane.size(); i++) {
            shared_ptr<Profesor> profesorPtr = dynamic_pointer_cast<Profesor>(persoane[i]);
            if (profesorPtr){
...
*/    void afisDiscipline(){
    ...
    void modifySalariu(float procent){
...
void afisNrStudenti(Facultate* facultati, int& ctr){
    cout<<Student::getCount()<<" studenti\n";
}
